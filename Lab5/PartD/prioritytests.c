...
CTEST2(priority3, test_wait) {
    int wait_times[] = {2,0,6};
    for (int i = 0; i < data->size; i++) {
        ASSERT_EQUAL(wait_times[i], (int)data->task[i].waiting_time);
        ASSERT_EQUAL(1, (float)calculate_average_wait_time(data->task, data->size) == (float)(8./3));
...
CTEST2(priority3, test_turn) {
    int turn_times[] = {6,2,12};
    for (int i = 0; i < data->size; i++) {
        ASSERT_EQUAL(turn_times[i], (int)data->task[i].turnaround_time);
        ASSERT_EQUAL(1, ((float)calculate_average_turn_around_time(data->task, data->size) == (float)(20.0/3)));
    }
}
